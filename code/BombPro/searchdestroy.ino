void search(){
  ...
  while(1){  // this is the important code, is a little messy but works good.
    ...
    int botonArmadoFM = analogRead(ARMARFM);                                          //Coloco una variable para almacenar el dato de FM, si es alto activa el armando
    int botonArmadoFM1 = analogRead(ARMARFM1);                                          //Coloco una variable para almacenar el dato de FM, si es alto activa el armando
    while(defuseando || botonArmadoFM>0 || botonArmadoFM1>200  && !passwordEnable)
    {
      ...
      while(defuseando || botonArmadoFM>0 || botonArmadoFM1>200)                                          //COMPRUEBO EL PULSADOR Y EL FM
      {
        keypad.getKey();
...
void destroy(){
  ...
  while(1){
    ...
    int botonDesarmadoFM = analogRead(DESARMARFM);                                          //Coloco una variable para almacenar el dato de FM, si es alto activa el armando
    int botonDesarmadoFM1 = analogRead(DESARMARFM1);                                          //Coloco una variable para almacenar el dato de FM, si es alto activa el armando

    if(defuseando || botonDesarmadoFM>500 || botonDesarmadoFM1>500 && !passwordEnable)// disarming bomb                 //Compruebo que si esta pulando o si pulsa e
    {
      lcd.clear();
      digitalWrite(REDLED, LOW);  
      lcd.setCursor(3,0);
      ...
      while(defuseando || botonDesarmadoFM>500 || botonDesarmadoFM1>500)
      {
        keypad.getKey();
