...
// Si se inicia un nuevo juego, debemos reiniciar correctamente estas variables
  saStatus=false;
  sdStatus=false;
  doStatus=false;
  
//Mostramos el menú
  cls();          //limpiamos la lcd
  int i=0;
  
//AQUÍ PUEDE AÑADIR MÁS ARTÍCULOS EN EL MENÚ PRINCIPAL
  lcd.print(menu1[i]);
  lcd.setCursor(15,1);
  checkArrows(i,2);
  while(1){
...
//Activamos el Mosfet y los malos ganan =(
  if(sdStatus || saStatus){
    cls();
...
//Puedes meter la contraseña y desarmar la bomba
  if(sdStatus || saStatus){
    cls();
